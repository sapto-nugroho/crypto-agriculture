...
def send_to_server_with_response(packet):
    ...
    """
    try:
        resp = requests.post(f"{SERVER_URL}/store",
...
def receive_from_sensor():
    ...
    # Kirim ke server, catat waktu transmisi
    ...
    if server_response:
        ...
        total_ms  = round(enc_ms + trans_ms + dec_ms, 4)
        stats["sent"] += 1
        print(f"[SENT] seq#{str(sensor_data['sequence_number']).zfill(5)} | "
              f"enc={enc_ms}ms | trans={trans_ms}ms | "
              f"dec={dec_ms}ms | total={total_ms}ms")

        # Kembalikan semua waktu ke experiment.py
        return jsonify({
            "status":   "ok",
            "enc_ms":   enc_ms,
            "trans_ms": trans_ms,
